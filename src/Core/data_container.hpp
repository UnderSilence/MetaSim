...
public:
  ...
  public:
    ...
    RangeSet::const_iterator ranges_end;
    RangeSet::const_iterator ranges_begin;
    ...
    iterator(RangeSet::iterator , int data_id)
      : ranges_iter(self.sub_ranges.cbegin())
      , ranges_begin(self.sub_ranges.cbegin()) {
      // initialize
      iterators = std::apply([](auto&&... args) { return iterator_set_type(args.begin()...); },
                             self.array_set);
    ...
    void advance_iterators_to(int dst_entry_id) {
      std::apply([&](auto&&... iters) { ((iters.advance_entry_to(dst_entry_id)), ...); },
                 iterators);
  ...
  auto begin() const { return const_iterator(sub_ranges.cbegin(), begin_entry, array_set); }
  auto end() { return iterator(sub_ranges.begin(), end_entry, array_set); }
  auto end() const { return const_iterator(sub_ranges.cbegin(), end_entry, array_set); }
};
